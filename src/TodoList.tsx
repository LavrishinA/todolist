...
export function Todolist(props: PropsType) {
    ...
    let filtredTask = props.tasks;

    if (filter === "active") {
        filtredTask = props.tasks.filter(t => t.isDone === false);
    }
    if (filter === "completed") {
        filtredTask = props.tasks.filter(t => t.isDone === true);
    }
    ...
    return <div>
        ...
        <ul>
            {
                filtredTask.map(t => <li key={t.id}>
                    <input type="checkbox" checked={t.isDone}/>
                    <span>{t.title}</span>
                    <button onClick={ () => { props.onRemoveTask(t.id) } }>x</button>
                </li>)
            }
        </ul>
        <div>
